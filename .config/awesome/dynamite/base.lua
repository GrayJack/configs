...
local l_wrapper  = require( "dynamite.widget.placeholder" )
local xresources = require( "beautiful.xresources"        )
local unpack     = unpack or table.unpack -- luacheck: globals unpack (compatibility with Lua 5.1)

local internal = {}

-- The hierarchy contexts
local contexts = setmetatable({}, {__mode = "k"})

-- Create and return the screens context
local function get_context(t)
    local s = capi.screen[t.screen or 1]
...
local function insert_wrapper(handler, c, wrapper, in_place_of)
    ...
    if in_place_of then
        -- Technically, it's not mandatory, so the check is needed.
        -- The fixed.replace_widget is generic, it doesn't use `self`.
        local f = handler.widget.replace_widget or wfixed.replace_widget
        f(handler.widget, in_place_of, wrapper, true)
    else
...
local function get_client_differential(self)
    ...
    for _,c in ipairs(clients) do
        -- `get()` is ON PURPOSE. It is *NOT* equal to `c.floating` in this case
        -- because immobilized/maximized/fullscreen clients count as floating too.
        -- They have to be handled by the layout *anyway* because they need to
        -- have a placeholder so they are restored in the right spot.
        local is_tiled = aw_client.property.get(c, "floating") or true

        if is_tiled and not self.client_to_wrapper[c] then
            table.insert(c.minimized and minimized or added, c)
        end
        reverse[c] = true
    end
    ...
    for c,_ in pairs(self.client_to_wrapper) do
        ...
        if ((not is_valid) or ((not reverse[c])) and not c.minimized) then
            table.insert(removed, c)
        end
    end

    return added, removed, minimized
end
...
local function wake_up(self)
    ...
    local added, removed, minimized = get_client_differential(self)
    ...
    for _, c in ipairs(added) do
        if not c.floating then
            if self.client_to_wrapper[c] then
                ...
                insert_wrapper(self, c, l_wrapper(c))
            end
        end
    end

    -- Add placeholders for minimized and shaded clients
    for _, c in ipairs(added) do
        --
    end
...
local function redraw(_, handler)
    if handler.active then
        ...
        handle_hierarchy(get_context(handler._tag), m, handler.hierarchy, wa)
    end
end
...
function internal.create_layout(t, l)
    ...
    function handler.arrange(param)
        ...
        if handler.honor_workarea then
            local wa = handler._tag.screen.workarea
            assert(wa.x <= screen_geo.x)
            assert(wa.y <= screen_geo.y)
            assert(wa.x+wa.width >= screen_geo.x+screen_geo.width)
            assert(wa.y+wa.height >= screen_geo.y+screen_geo.height)
        end
...
-- Swap a placeholder with a client.
-- There is a race condition where the layout could have been changed before
-- the client appears. There isn't much to do about it.
function public._place_client(c, t, widget)
    ...
    if c then
        insert_wrapper(self, c, l_wrapper(c), widget)
    else
        local f = self.remove_widgets or wfixed.remove_widgets
        f(self, widget, true)
        return
    end
end

return setmetatable(public, { __call = function(_, ...) return register(...) end })
