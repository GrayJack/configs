...
-- Check of the new elements have a `ratio` property.
function methods:add(...)
    local ret = self._private.original_add(self, ...)

    --TODO
--     for _, widget in ipairs {...} do
--         print("RAT", widget, widget.children, widget.ratio)
--     end

    return ret
end

local function resize(self, origin, request)
    print("\n\nRATIO", origin, request)
end
...
local function get_layout(dir, widget1, ...)
    ...
    orig._private.original_add = orig.add
    ...
    ret:connect_signal("request::resize", function(...) resize(ret, ...) end)
